...
Changelog:
- Fixed: Wrap attributes
- Added: Number of replacements performed
*/
	...
	if( key_exists( 'mfn_migrate_nonce',$_POST ) ) {
		if ( wp_verify_nonce( $_POST['mfn_migrate_nonce'], basename(__FILE__) ) ) {
			...
			if( strlen($old_url) < $safety_limit || strlen($new_url) < $safety_limit ){
				...
				echo '<div class="notice notice-error"><p>For your own safety please use URLs longer than '. $safety_limit .' characters !</p></div>';
				
			} elseif( strpos( $old_url, 'http' ) !== 0 || strpos( $new_url, 'http' !== 0 )  ){

				echo '<div class="notice notice-error"><p>URLs must begin with http:// or https:// !</p></div>';
					...
					if( $data && is_array( $data ) ){
						foreach( $data as $key => $option ){
							if( is_string( $option ) ){
								...
								$data[$key] = str_replace( $old_url, $new_url, $option, $count );
								$replacements += $count;
							...
							foreach( $meta_value as $sec_key => $sec ){
								...
								if( isset( $sec['attr'] ) && is_array( $sec['attr'] ) ){
									foreach( $sec['attr'] as $attr_key => $attr ){
										...
										$attr = str_replace( $old_url, $new_url, $attr, $count );
										$replacements += $count;
														...
														if( $field_key == 'tabs' ) {
															...
															if( isset( $field ) && is_array( $field ) ){
																foreach( $field as $tab_key => $tab ){
																	...
																	$field = str_replace( $old_url, $new_url, $tab['content'], $count );
																	$replacements += $count;
															...
															$field = str_replace( $old_url, $new_url, $field, $count );
															$replacements += $count;
...
// 						print_r($meta_value);
				...
				echo '<div class="notice notice-success">';
					echo '<p><b>All done. Have fun!</b><br />Replacements performed: '. $replacements .'</p>';
				echo '</div>';
